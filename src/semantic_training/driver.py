...
def writeToTensorBoard(writer, tensorboardData, curr_episode):
    # tensorboardData: [reward, loss_q, loss_pi, loss_alpha, entropy, alpha, q_val, 
    #                   avg_reward, avg_dist, success_rate]
    tensorboardData = np.array(tensorboardData)
    # 取平均值，忽略 NaN
    avg_data = np.nanmean(tensorboardData, axis=0)
    
    # 解包数据 (根据 training_data 的 append 顺序)
...
def main():
    ...
    # Experience Buffer (20 channels based on worker output)
    experience_buffer = [] 
    for _ in range(20): 
        experience_buffer.append([])

    # Metrics containers
    training_data = [] # Stores [loss_q, loss_pi, ...] for tensorboard
    ...
    try:
        ...
        while True:
            # 1. Asynchronous Wait: Wait for at least 1 job to finish
            # This improves GPU utilization compared to waiting for all
            done_id, job_list = ray.wait(job_list, num_returns=1)
            done_jobs = ray.get(done_id)
            
            # 2. Process Result & Launch New Job
            for job in done_jobs:
                ...
                # Launch replacement job immediately
                agent_id = info['id']
                
                # Determine Dataset (Check again for dynamic switching)
                if curr_episode < DATASET_EASY_EPOCHS:
            ...
            if buffer_size > REPLAY_SIZE:
                ...
                buffer_size = len(experience_buffer[0]) # Update size
            ...
            if buffer_size >= MINIMUM_BUFFER_SIZE:
                ...
                def run_update_step():
                    ...
                    with torch.no_grad():
                        ...
                        target_q_min = torch.min(target_q1_val, target_q2_val.to(device))
                        
                        # Soft V Calculation
                        next_v = (next_probs * (target_q_min - alpha * next_logp_list)).sum(dim=1, keepdim=True)
                    ...
                    # Gather Q values corresponding to action
                    # [FIXED]: Ensure dimensions are (Batch, 1) to avoid broadcasting
                    current_q1 = torch.gather(current_q1_val, 1, b_action.squeeze(-1)) 
                    current_q2 = torch.gather(current_q2_val, 1, b_action.to(dev_q2).squeeze(-1))
                    
                    # MSE Loss (Ensure target_q is squeezed to match current_q1: (Batch, 1))
                    loss_q = nn.MSELoss()(current_q1, target_q.squeeze(-1)) + \
                             nn.MSELoss()(current_q2.to(device), target_q.squeeze(-1))
                    ...
                    with torch.no_grad():
                        ...
                        min_q = torch.min(q1_val, q2_val.to(device))
                    ...
                    return loss_q.item(), loss_policy.item(), loss_alpha.item(), entropy.item(), log_alpha.exp().item(), current_q1.mean().item()
                ...
                if step_results:
                    avg_res = np.mean(step_results, axis=0) # [loss_q, loss_pi, loss_alpha, entropy, q_val]
                    
                    # Prepare data for tensorboard [losses..., perf_metrics...]
                    # We use the metrics from the job that just finished to represent performance
                    current_perf = [
                    ...
                    # Combined data: Reward (from batch), losses..., performance metrics
                    # Note: We use b_reward.mean() from the batch usually, but here let's use the episode reward from metrics
            ...
            if curr_episode % SUMMARY_WINDOW == 0 and len(training_data) > 0:
                ...
                # Check for best model based on Semantic Gain (index -3 in training_data)
            ...
            if curr_episode % 200 == 0:
                ...
                torch.save(save_dict, f'{model_path}/checkpoint_{curr_episode}.pth')
