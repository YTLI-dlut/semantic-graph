...
def writeToTensorBoard(writer, tensorboardData, curr_episode):
    ...
    reward, value, policyLoss, qValueLoss, entropy, policyGradNorm, qValueGradNorm, log_alpha, alphaLoss, travel_dist, success_rate, explored_rate, total_steps, expolred_IOU = tensorboardData
    ...
    writer.add_scalar(tag='Perf/Explored IOU', scalar_value=expolred_IOU, global_step=curr_episode)
...
def main():
    ...
    global_q_net1 = QNet(INPUT_DIM + int(USE_K_FLAGS) * N_ROBOTS, EMBEDDING_DIM).to(device)
    global_q_net2 = QNet(INPUT_DIM + int(USE_K_FLAGS) * N_ROBOTS, EMBEDDING_DIM).to(device)
    log_alpha = torch.FloatTensor([-2]).to(device)  # not trainable when loaded from checkpoint, manually tune it for now
    log_alpha.requires_grad = True

    global_target_q_net1 = QNet(INPUT_DIM + int(USE_K_FLAGS) * N_ROBOTS, EMBEDDING_DIM).to(device)
    global_target_q_net2 = QNet(INPUT_DIM + int(USE_K_FLAGS) * N_ROBOTS, EMBEDDING_DIM).to(device)
    ...
    metric_name = ['travel_dist', 'success_rate', 'explored_rate', 'total_steps', 'explored_IOU']
    training_data = []
    perf_metrics = {}
        ...
        while True:
            ...
            if curr_episode % 1 == 0 and len(experience_buffer[0]) >= MINIMUM_BUFFER_SIZE:
                ...
                for j in range(8):
                    ...
                    utility_mask_batch = torch.stack(rollouts[15]).to(device)
                    next_utility_mask_batch = torch.stack(rollouts[16]).to(device)

                    q_net_node_inputs_batch = node_inputs_batch
                    q_net_next_node_inputs_batch = next_node_inputs_batch
                    # SAC
                    with torch.no_grad():
                        # print('node_inputs_batch', q_net_node_inputs_batch.shape, edge_inputs_batch.shape, current_inputs_batch.shape, node_padding_mask_batch.shape, edge_padding_mask_batch.shape, edge_mask_batch.shape)
                        q_values1, _ = dp_q_net1(q_net_node_inputs_batch, edge_inputs_batch, current_inputs_batch, node_padding_mask_batch, edge_padding_mask_batch, edge_mask_batch, utility_mask_batch)
                        q_values2, _ = dp_q_net2(q_net_node_inputs_batch, edge_inputs_batch, current_inputs_batch, node_padding_mask_batch, edge_padding_mask_batch, edge_mask_batch, utility_mask_batch)
                        q_values = torch.min(q_values1, q_values2)

                    logp = dp_policy(node_inputs_batch, edge_inputs_batch, current_inputs_batch, node_padding_mask_batch, edge_padding_mask_batch, edge_mask_batch, utility_mask_batch)
                    # print("logp", logp.shape, q_values.shape, log_alpha.shape)
                    ...
                    with torch.no_grad():
                        next_logp = dp_policy(next_node_inputs_batch, next_edge_inputs_batch, next_current_inputs_batch, next_node_padding_mask_batch, next_edge_padding_mask_batch, next_edge_mask_batch, next_utility_mask_batch)
                        next_q_values1, _ = dp_target_q_net1(q_net_next_node_inputs_batch, next_edge_inputs_batch, next_current_inputs_batch, next_node_padding_mask_batch, next_edge_padding_mask_batch, next_edge_mask_batch, next_utility_mask_batch)
                        next_q_values2, _ = dp_target_q_net2(q_net_next_node_inputs_batch, next_edge_inputs_batch, next_current_inputs_batch, next_node_padding_mask_batch, next_edge_padding_mask_batch, next_edge_mask_batch, next_utility_mask_batch)
                        ...
                        # print("target q batch", target_q_batch.shape)
                    mse_loss = nn.MSELoss()
                    q_values1, _ = dp_q_net1(q_net_node_inputs_batch, edge_inputs_batch, current_inputs_batch, node_padding_mask_batch, edge_padding_mask_batch, edge_mask_batch, utility_mask_batch)
